...
class OrganizationController {
	...
	def show() {
		...
		if (organizationInstance.salesStatus == "LEAD") {
			redirect controller:'lead', action: 'show',params:[id : organizationInstance?.id]
		}
	...
	def delete() {
		...
		try {
			...
			messages << message(code: 'default.deleted.message', args: [
				message(code: 'organization.label', default: 'Organization'),
				params.id
			])
			render(contentType: "text/json") {
			...
			messages << message(code: 'default.not.deleted.message', args: [
				message(code: 'organization.label', default: 'Organization'),
				params.id
			])
			render(contentType: "text/json") {
	...
	def createPhoneBook() {
		switch (request.method) {
			...
			case 'POST':
				...
				flash.message = message(code: 'default.created.message', args: [
					message(code: 'phoneBook.label', default: 'PhoneBook'),
					phoneBookInstance.id
				])
