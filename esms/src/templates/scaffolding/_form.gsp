...
private renderFieldForProperty(p, owningClass, prefix = "") {
	...
	if (display) { %>
			<div class="form-group fieldcontain \${hasErrors(bean: ${propertyName}, field: '${prefix}${p.name}', 'error')} ${required ? 'required' : ''}">
				<label for="${prefix}${p.name}" class="col-md-3 col-sm-3 control-label"><g:message code="${domainClass.propertyName}.${prefix}${p.name}.label" default="${p.naturalName}" /><% if (required) { %><span class="required-indicator">*</span><% } %></label>
				<div class="col-md-9">
					${renderEditor(p)}
